...
/**
 * Manages request rate limiting based on IP addresses using a sliding
 * expiration cache.
 * <p>
 * This class utilizes a Caffeine cache to store and expire request counters for
 * each unique IP.
 * It provides a basic leaky-bucket or fixed-window style filtering to prevent
 * abuse of the API.
 * </p>
 */
public class RateLimitManager {
   ...
   * <p>
   * Keys are IP strings, Values are AtomicIntegers representing the request
   * count.
   * Entries expire 1 minute after creation (fixed window).
   * </p>
   */
   ...
   *
   * @param ip   The IP address of the requester.
   * @param cost The "weight" of the request (e.g., login attempts are more
   *             expensive).
